...
INSERT INTO estado_tramo (nombre, descripcion) VALUES
  ('PENDIENTE', 'El tramo está estimado pero aún no ha comenzado'),
  ('EN_CURSO',  'El tramo se encuentra actualmente en ejecución'),
  ('FINALIZADO','El tramo ha sido completado');

-- Tipos de tramo de acuerdo con el enunciado (origen‑depósito, depósito‑depósito, depósito‑destino, origen‑destino)
INSERT INTO tipo_tramo (nombre, descripcion) VALUES
  ('ORIGEN_A_DEPOSITO',    'Traslado desde el origen hasta el primer depósito'),
  ('DEPOSITO_A_DEPOSITO',  'Traslado entre depósitos intermedios'),
  ('DEPOSITO_A_DESTINO',   'Traslado desde el último depósito al destino final'),
  ('ORIGEN_A_DESTINO',     'Traslado directo desde el origen al destino, sin depósitos');
...
INSERT INTO solicitud (cliente_id, contenedor_id, tarifa_id, costo_estimado, tiempo_estimado) VALUES
  (1, 1, 1, 5000.00, 15.5),
  (2, 2, 2, 8000.00, 20.0);

-- Rutas asociadas a cada solicitud (1:1)
INSERT INTO ruta (cantidad_tramos, cantidad_depositos, id_solicitud) VALUES
  (1, 2, 1),
  (2, 3, 2);
